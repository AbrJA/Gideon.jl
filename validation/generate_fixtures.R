...
write_triplet(X, "test/fixtures/X_small.csv")
write.csv(data.frame(nr = n_u, nc = n_i),
          "test/fixtures/X_small_dims.csv", row.names = FALSE)
cat(sprintf("   nnz=%d\n", nnzero(X)))
...
write.csv(U_chol, "test/fixtures/wrmf_chol_user.csv", row.names = FALSE)
write.csv(V_chol, "test/fixtures/wrmf_chol_item.csv", row.names = FALSE)

r_loss_chol <- obs_loss(U_chol, V_chol, X, LAMBDA, ALPHA)
writeLines(as.character(r_loss_chol), "test/fixtures/wrmf_chol_loss.txt")
...
write.csv(scores_chol, "test/fixtures/wrmf_chol_scores_top10.csv",
          row.names = FALSE)
...
write.csv(U_cg, "test/fixtures/wrmf_cg_user.csv", row.names = FALSE)
write.csv(V_cg, "test/fixtures/wrmf_cg_item.csv", row.names = FALSE)

r_loss_cg <- obs_loss(U_cg, V_cg, X, LAMBDA, ALPHA)
writeLines(as.character(r_loss_cg), "test/fixtures/wrmf_cg_loss.txt")
cat(sprintf("   obs-entry loss: %.6f\n", r_loss_cg))
...
write_triplet(as(X_ftrl, "CsparseMatrix"), "test/fixtures/X_ftrl.csv")
write.csv(data.frame(nr = n_f, nc = p_f),
          "test/fixtures/X_ftrl_dims.csv", row.names = FALSE)
write.csv(data.frame(y = y_ftrl), "test/fixtures/y_ftrl.csv", row.names = FALSE)
...
write.csv(data.frame(w = w_r), "test/fixtures/ftrl_weights.csv", row.names = FALSE)
write.csv(data.frame(p = p_r), "test/fixtures/ftrl_preds.csv",   row.names = FALSE)
...
write.csv(data.frame(p = p_fm), "test/fixtures/fm_xor_preds.csv",
          row.names = FALSE)
...
write_triplet(co, "test/fixtures/glove_X.csv")
write.csv(data.frame(nr = 50L, nc = 50L),
          "test/fixtures/glove_dims.csv", row.names = FALSE)
...
writeLines(as.character(final_cost), "test/fixtures/glove_final_cost.txt")
# Save full cost history for monotonicity comparison
write.csv(data.frame(cost = cost_hist), "test/fixtures/glove_costs.csv",
          row.names = FALSE)
...
write.csv(data.frame(ap = ap_val, ndcg = ndcg_val),
          "test/fixtures/metrics_ref.csv", row.names = FALSE)
cat(sprintf("   AP@4=%.6f  NDCG@4=%.6f\n", ap_val, ndcg_val))

# ── Summary ───────────────────────────────────────────────────────────────────
cat("\nFixtures written to test/fixtures/:\n")
for (f in sort(list.files("test/fixtures"))) {
  sz <- file.info(file.path("test/fixtures", f))$size
