...
@testset "Basic fit" begin
    ...
    model = ImplicitALS(rank=8, λ=0.01, α=40.0, max_iter=5, verbose=false)
    fit!(model, X; rng=rng)
...
@testset "Loss decreases" begin
    ...
    model = ImplicitALS(rank=5, λ=0.1, α=10.0, max_iter=10, convergence_tol=-1.0, verbose=false)
    fit!(model, X; rng=rng)
    @test model.is_fitted
end
...
@testset "recommend returns valid indices" begin
    ...
    model = ImplicitALS(rank=5, λ=0.01, α=10.0, max_iter=5, verbose=false)
    fit!(model, X; rng=rng)
...
@testset "score pairwise" begin
    ...
    model = ImplicitALS(rank=4, λ=0.01, α=10.0, max_iter=3, verbose=false)
    fit!(model, X; rng=rng)
...
@testset "Warm start" begin
    ...
    model = ImplicitALS(rank=k, λ=0.01, α=10.0, max_iter=3, verbose=false)
...
@testset "Convergence tolerance" begin
    ...
    model = ImplicitALS(rank=5, λ=0.01, α=10.0, max_iter=100, convergence_tol=0.01, verbose=false)
    fit!(model, X; rng=rng)
    @test model.is_fitted
end
