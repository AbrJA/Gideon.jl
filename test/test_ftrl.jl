...
@testset "Basic fit and predict (binomial)" begin
    model = FTRL(learning_rate=0.1, λ=0.01, l1_ratio=0.5, family=BINOMIAL, verbose=false)
...
@testset "Loss decreases with more epochs" begin
    m1 = FTRL(learning_rate=0.1, λ=0.01, l1_ratio=0.5, verbose=false)
    partial_fit!(m1, X, y; rng=MersenneTwister(1))
    m5 = FTRL(learning_rate=0.1, λ=0.01, l1_ratio=0.5, verbose=false)
    for _ in 1:5; partial_fit!(m5, X, y; rng=MersenneTwister(1)); end
    @test logloss(predict(m5, X), y) < logloss(predict(m1, X), y)
end

@testset "L1 regularization zeroes weights" begin
    m_l1 = FTRL(learning_rate=0.1, λ=2.0, l1_ratio=1.0, verbose=false)
    for _ in 1:5; partial_fit!(m_l1, X, y); end
    nnz_w = sum(abs.(coef(m_l1)) .> 1e-10)
    @test nnz_w < p
end

@testset "partial_fit! updates weights" begin
    model2 = FTRL(learning_rate=0.1, verbose=false)
...
@testset "Gaussian family" begin
    ...
    model = FTRL(learning_rate=0.01, λ=0.001, family=GAUSSIAN, verbose=false)
    fit!(model, X, y_cont; n_iter=3)
    preds = predict(model, X)
    @test length(preds) == n
...
@testset "Poisson family" begin
    ...
    model = FTRL(learning_rate=0.01, λ=0.001, family=POISSON, verbose=false)
    fit!(model, X, y_count; n_iter=3)
    preds = predict(model, X)
    @test length(preds) == n
...
@testset "Gradient clipping" begin
    ...
    model = FTRL(learning_rate=0.1, clip_gradient=10.0, verbose=false)
    fit!(model, X_large, y_bin; n_iter=3)
    @test all(isfinite, coef(model))
    @test all(isfinite, predict(model, X_large))
end

@testset "Dropout" begin
    model = FTRL(learning_rate=0.1, dropout=0.3, verbose=false)
    fit!(model, X, y; n_iter=3, rng=MersenneTwister(42))
    preds = predict(model, X)
    @test all(0 .<= preds .<= 1)
end
...
@testset "Edge cases" begin
    ...
    y_single = [1.0]
    model = FTRL(learning_rate=0.1, verbose=false)
    partial_fit!(model, X_single, y_single)
    @test model.is_initialized
