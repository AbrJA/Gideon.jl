...
# Aggressive huge-matrix benchmark for Gideon.jl WMF
...
function bench_wrmf(X::SparseMatrixCSC, solver::ALSSolver;
    ...
    minimum(
        @elapsed(fit!(WMF(rank=rank, λ=lambda, α=alpha, max_iter=n_iter,
...
let X_w = make_sparse_matrix(300, 200, 0.1; seed=1)
    for s in (CHOLESKY, ConjugateGradient())
        fit!(WMF(rank=10, λ=0.1, α=1.0, max_iter=3, solver=s, feedback=IMPLICIT),
